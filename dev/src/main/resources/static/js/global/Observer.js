...
export class DynamicDomRenderer {
    ...
    $template;
    constructor() {
        this.$template = $($('#template')[0].content);
    }
    static getInstance() {
            ...
            mutations.forEach((mutation) => {
                if (mutation.type === 'childList' && mutation.addedNodes.length) {
                    mutation.addedNodes.forEach((node) => {
                        if ($(node).is('[data-custom-svg]') || $(node).is('[data-custom-img]'))
                            this.handleCustomElementInjection(node);
                        if ($(node).is(`[data-css-row]`) || $(node).is(`[data-css-column]`))
                            this.handleCustomGridStyles(node);
                    });
                }
            });
        });
    ...
    handleCustomElementInjection(node) {
        try {
    ...
    handleCustomGridStyles(node) {
        try {
            ...
            if (row) {
                ...
                if (template)
                    css['grid-template-rows'] = template.split(':').reduce((acc, u) => {
                        if (u.includes('fr') || u.includes('auto'))
                            acc += `${u} `;
                        else if (u.includes('rep'))
                            acc += `repeat(${u.split('/')[1]}, ${Number(size) * Number(u.split('/')[2])}px) `;
                        else
                            acc += `${Number(size) * Number(u)}px `;
                        return acc;
                    }, '');
                if (gap)
                    css['row-gap'] = `${gap}px`;
            ...
            if (column) {
                ...
                if (template)
                    css['grid-template-columns'] = template.split(':').reduce((acc, u) => {
                        if (u.includes('fr') || u.includes('auto'))
                            acc += `${u} `;
                        else if (u.includes('rep'))
                            acc += `repeat(${u.split('/')[1]}, ${Number(size) * Number(u.split('/')[2])}px) `;
