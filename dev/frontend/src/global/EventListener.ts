...
export class DynamicDomListener {
      ...
      async initializeEventListener() {
            //? action:oneway
            $(document).on('click', `[data-action-type*="oneway"]`, async (e: any) => {
            ...
            //? action:toggle
            $(document).on('click', `[data-action-type*="toggle"]`, async (e: any) => {
                  const $btn = $(e.target).closest(`[data-action-type]`);
                  const template = $btn.data('action-template')?.split(':') ?? [];
                  const [on, off, key] = template;
                  if (!on || !off || !key) return;
                  const $target = $(`[data-action-for="${key}"]`);
            ...
            //? action:selected
            $(document).on('click', `[data-action-type*="selected"]`, async (e: any) => {
                  const $btn = $(e.target).closest(`[data-action-type]`);
